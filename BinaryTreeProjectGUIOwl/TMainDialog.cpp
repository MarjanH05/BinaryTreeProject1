...
void TMainDialog::SetupWindow()
{
    ...
    HRSRC hRes = ::FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_COMPUTAIRLOGO), RT_BITMAP);
    if (!hRes) {
        ::MessageBox(NULL, TEXT("Bitmap resource not found!"), TEXT("Error"),
            MB_OK | MB_ICONERROR);
        return;
    }
    ...
    HBITMAP hBitmap = (HBITMAP)::LoadImage(
        ...
        MAKEINTRESOURCE(IDB_COMPUTAIRLOGO),
        IMAGE_BITMAP,
        800, 200,
    ...
    if (!hBitmap) {
        ::MessageBox(NULL, TEXT("Failed to load bitmap!"), TEXT("Error"),
            MB_OK | MB_ICONERROR);
        DWORD error = GetLastError();
        TCHAR errorMsg[256];
        FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, error, 0, errorMsg, 256, NULL);
        ::MessageBox(NULL, errorMsg, TEXT("LoadImage Error"), MB_OK | MB_ICONERROR);
    }
    else {
        HWND hStatic = GetDlgItem(IDC_LOGO_STATIC);
        if (hStatic) {
            ::SendMessage(hStatic, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBitmap);
        }
        else {
            ::MessageBox(NULL, TEXT("Static control not found!"), TEXT("Error"),
                MB_OK | MB_ICONERROR);
        }
    }
}
...
void TMainDialog::CmProcess()
{
    ...
    try {
        ...
        for (const auto& [expression, value] : results) {
            ...
            owl::tstring displayText = owl::tstring(expression.begin(), expression.end()) +
                _T(" = ") + owl::tstring(formattedResult);
        ...
        try {
            ...
            if (saveResponse == IDYES) {
                ...
                if (GetOpenFileNameStr(filePath)) {
                    ...
                    if (outFile.is_open()) {
                        outFile << std::wstring(equation.begin(), equation.end())
                            << _T(" = ") << result << std::endl;
                        MessageBox(_T("Result saved successfully!"),
