...
TEST_F(EvaluateTreeTest, MultiLevelExpression) {
    auto root = std::make_shared<TreeNode>("*");
    ...
    root->right->left = std::make_shared<TreeNode>("3");
    ...
    EXPECT_DOUBLE_EQ(result, 21.0); // (1 + 2) * (3 + 4)
}

TEST_F(EvaluateTreeTest, NestedExpression) {
    auto root = std::make_shared<TreeNode>("+");
    root->left = std::make_shared<TreeNode>("3");
    root->right = std::make_shared<TreeNode>("*");
    root->right->left = std::make_shared<TreeNode>("2");
    root->right->right = std::make_shared<TreeNode>("4");
    ...
    EXPECT_DOUBLE_EQ(result, 11.0); // 3 + (2 * 4)
}

TEST_F(EvaluateTreeTest, SimpleAddition) {
    auto root = std::make_shared<TreeNode>("+");
    root->left = std::make_shared<TreeNode>("3");
    ...
    EXPECT_DOUBLE_EQ(result, 8.0); // 3 + 5
}

TEST_F(EvaluateTreeTest, NegativeNumbers) {
    auto root = std::make_shared<TreeNode>("+");
    ...
    EXPECT_DOUBLE_EQ(result, 2.0); // -2 + 4
}
